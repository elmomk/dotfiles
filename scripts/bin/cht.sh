...
core_utils=$(echo "rsync  distrobox fd ripgrep pacman npm yay tmux vim nvim awk sed grep find sort uniq wc xargs bat fzf mv rm ssh systemctl docker kubectl terraform ansible scp" | tr ' ' '\n' | sort)
