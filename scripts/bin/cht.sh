...
core_utils=$(echo "nmcli dig git tr cut useradd usermod groupmod groupadd paru helm az rsync  distrobox fd ripgrep pacman npm yay tmux vim nvim awk sed grep find sort uniq wc xargs bat fzf mv rm ssh systemctl docker kubectl terraform ansible scp" | tr ' ' '\n' | sort)
...
if printf '%s' "$languages" | grep -qs "$selected"; then
  ...
  curl cht.sh/$selected/$query 2>/dev/null | bat &
